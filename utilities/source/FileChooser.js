...
enyo.kind({
	...
	selectedNameChanged:  function(oldSelectedName) {
		if (this.debug) {
			this.log("old:", oldSelectedName, " -> new:", this.selectedName);
		}
	...
	confirm: function(inSender, inEvent) {
		...
		this.hide() ;
		if (this.debug) {
			this.log("selectedFile:", this.selectedFile, "selectedName:", this.selectedName);
		}
		this.doFileChosen({
	...
	createFolder: function(inSender, inEvent) {
		...
		if (this.debug) {
			this.log("Creating new folder:", name, "into folderId:", folderId, "under service:", service);
		}

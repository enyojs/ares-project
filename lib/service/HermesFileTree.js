...
//			]},
		...
		{kind: "Scroller", fit: true, components: [
			...
			{kind: "Node", classes: "enyo-unselectable", showing: false, file: {id: ".", name: "server", isServer: true, service: null}, content: "server", icon: "$service/images/antenna.png", expandable: true, expanded: true, onExpand: "nodeExpand", onNodeTap: "nodeTap"}
		]},
	...
	create: function() {
		...
		this.$.service.auth = 0;
		this.$.service.setUrl(null);
		this.$.service.setUseJsonp(false);
	},
	setService: function(inData) {
		this.$.service.auth = inData.auth;
		this.$.service.setUrl(inData.url);
		this.$.service.setUseJsonp(inData.jsonp);		
	},
	reset: function() {
		this.$.node.hide();
		if (this.$.service.url) {
	...
	newFileConfirm: function(inSender, inEvent) {
		...
		r.response(this, function(inSender, inResponse) {
			...
			this.createFile(name, path, inResponse);
		});
		r.error(this, function(inSender, error) {
			console.log("error: "+error.toString());
			this.createFile(name, path, null);
		});
		r.go();
	},
	createFile: function(name, path, content) {
		this.log("Creating new file "+name+" at "+path);
		this.$.service.createFile(path, name, content)

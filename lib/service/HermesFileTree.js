enyo.kind({
	...
	components: [
		{kind: "onyx.Toolbar", classes: "onyx-menu-toolbar", components: [
			{name: "newFolder",content: "Files", style: "margin-right: 10px"},
	...
	setConfig: function(config) {
		if (config) {
			this.$.service.setServiceInformation(config.service);
		} else {
			this.$.service.setUrl(null);
		}
	}, 
	fileOps2Hide: function() {
		// disable all File/Folder Operations except New Folder
		this.$.newFile.hide();
		this.$.renameFile.hide();
		this.$.copyFile.hide();
		this.$.deleteFile.hide();
	}, 
	getSubFileView: function(inNodeName, inFolderId) {
		var serverNode = this.$.serverNode;	
		serverNode.hide();
		if (this.$.service.url) {
			if (inNodeName !== null) {
				serverNode.setContent(inNodeName);					
				serverNode.file = {id: inFolderId, name: inNodeName, isDir: true, isServer:true, path: inNodeName, service: null};
				this.$.selection.select(inFolderId, serverNode);
				serverNode.effectExpanded();
			}
			this.listFiles(serverNode);
		}         
	},

enyo.kind({
	...
	components: [
		...
		{kind: "Signals", onunload: "unloadHandler", onkeyup: "handleKeyUp" },
	...
	// See https://github.com/enyojs/enyo/wiki/User-Input
    handleKeyUp: function (inSender, inEvent) {
		// key code: see http://www.webonweboff.com/tips/js/event_key_codes.aspx
		if (inEvent.keyCode === 82) {
	...
	refreshFileTree: function(node) {
		...
		this.log('running refreshFileTree on ' + this.kind + ' with ' 
				 + target.controls.length + ' controls');
		enyo.forEach(target.controls, function(c) {
			this.log('running INNER function of refreshFileTree on ' + this.kind );

enyo.kind({
	...
	setConfig: function (inServiceId, inConfig) {
		...
		if (inConfig.auth && inConfig.auth !== service.config.auth) {
			service.config.auth = inConfig.auth;
			var key = [this.SERVICES_STORAGE_KEY, service.config.id].join('.');
			this.$.localStorage.set(key, JSON.stringify({auth: service.config.auth})); // should not really care of the completion callback
		}
	},
	/**
	...
	filter: function(criteria) {
		...
		var matches = enyo.filter(this.services, function(service){
			...
			if (this.debug) this.log("match:", match, "<= service:", service.conf);
			return match;
		}, this);
		var services = [];

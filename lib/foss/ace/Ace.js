	...
	*/
	...
	mapToLineColumns: function(streamPositions) {
		...
		while (lineIdx < lineCount) {
			...
			bytesCount += (nbBytes + 1);		// TODO ok with LF and CRLF
			// this.log("line: " + (lineIdx + 1) + " nb char: " + nbBytes + " total: " + bytesCount);
			lineIdx++;
		}
	...
	mapToLineColumnRange: function(start, end) {
		var pos;
		if (end === undefined) {
			pos = this.mapToLineColumns([start]);
			return this.createRange(pos[0].row, pos[0].column, pos[0].row, pos[0].column);
		} else {
			pos = this.mapToLineColumns([start, end]);

...
enyo.kind({
	...
	startLoading: function() {
		this.$.extra.setContent('&nbsp;<img src="' + enyo.path.rewrite("$assets/harmonia/services/images/busy.gif") + '"/>');
	},
	...
	filesToNodes: function(inFiles) {
		...
		for (var i=0; i < inFiles.length; i++) {
			...
			nodes.push({
				...
				content: f.name,
				expandable: f.isDir,
				icon: "$assets/harmonia/services/images/" + (f.isDir ? "arrowRight.png" : "file.png")
			});
		}
		return nodes;
	},
	nodeExpand: function(inSender, inEvent) {
		...
		if (subnode.file.isDir) {
			subnode.setIcon("$assets/harmonia/services/images/" + (subnode.expanded ? "arrowDown.png" : "arrowRight.png"));
		}
